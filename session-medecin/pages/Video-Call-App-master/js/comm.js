...
// if(location.protocol === 'http:'){
    // wsChat = new WebSocket("ws://127.0.0.1:8080/comm");
// }

// else{
    const wsChat = new WebSocket("ws://localhost:1337/comm");
// }
...
window.addEventListener('load', function(){
    ...
    wsChat.onmessage = function(e){
        ...
        if(data.room === room){
            ...
            switch(data.action){
                ...
                case 'typingStatus':
                    if(data.status){
                        document.getElementById("typingInfo").innerHTML = remoteName+" is typing";
                    }
                ...
                case 'newSub':
                    ...
                    showSnackBar(remoteName+" entered room", 10000);
                ...
                case 'imOffline':
                    ...
                    showSnackBar(remoteName+" left room", 10000);
                    enableCallBtns();
                    break;
            }  
        }
    ...
    */
    ...
    document.getElementById("chatInput").addEventListener('keypress', function(e){
        ...
        if((e.which === 13) && msg){            
					...
					$.ajax({
						type:'POST',																	
						url: "../PhpSocket2/ChatRequette.php",
						dataType: "json",   
						cache:false,
						data: formdata,						
						processData: false,
    ...
    */
    ...
    document.getElementById("rejectCall").addEventListener('click', function(e){
        ...
        wsChat.send(JSON.stringify({
            action: 'callRejected',
            msg: "Call rejected by "+remoteName,
            room: room
        }));
    ...
    */
    ...
    document.getElementById("endCall").addEventListener('click', function(e){
        ...
        endCall("Call ended by "+remoteName, false);
...
function initCall(){    
    ...
    if(checkUserMediaSupport){
        ...
        callerInfo.innerHTML = callType === 'Video' ? 'Video call to '+remoteName : 'Audio call to '+remoteName;
        ...
        wsChat.send(JSON.stringify({
            action: 'initCall',
            msg: callType === 'Video' ? "Video call from "+remoteName : "Audio call from "+remoteName,
            room: room
        }));
...
function answerCall(){
    ...
    if(checkUserMediaSupport){
        ...
        wsChat.send(JSON.stringify({
            action: 'callRejected',
            msg: remoteName+"'s device does not have the necessary requirements to make call",
            room: room
        }));
...
function handleCallTermination(){
    ...
    showSnackBar("Call terminated by "+remoteName, 10000);
