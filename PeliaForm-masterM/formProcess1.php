...
function test_input($data) {
  ...
  $result = $bdd->query('SELECT * FROM medecins');
      ...
      if($phone == $key['phone']){
        ...
        // sending Data to the database after confirming that thers no error
            ...
            if(!empty($lastname) && !empty($name) && !empty($email) && !empty($password) && !empty($cpassword) && !empty($phone) && !empty($sexe) && !empty($age) && $error==false){
                ...
                $req = $bdd->prepare('INSERT INTO medecins(prenom, nom, email, password, cpassword, phone, sex, age) VALUES(:lastname, :name, :email, :password, :cpassword, :phone, :sexe, :age)');
